...
int idx = myarrlist.indexOf("test")
println("the index of 'test' is: " + idx)
println("------------------------")
...
println("------------------------")
# use of a java.util.Clanendar java class
...
println("date: " + mydate)
println("------------------------")
# use a java.util.HashMap java class
...
int tIdx1 = Thread("myThreadHello", 2000)
if(tIdx1 != -1)
	println("Thread started with id: " + tIdx1)
else
	println("Thread error")
fi
int tIdx2 = Thread("myThreadMessage", 7000, 3)
if(tIdx2 != -1)
